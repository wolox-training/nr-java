...
public class BookControllerTest {
	...
	public void givenBook_whenGetBookByTitle_thenReturnBook() throws Exception {
		...
		mvc.perform(get("/api/books").param("title", title)).andDo(print()).andExpect(status().isOk())
...
//				.andExpect(jsonPath("$?.subtitle == 'Homeland'").value("Homeland"));
	...
	public void givenListOfBooks_whenGetBooksByTitle_thenFindBookBySubtitle() throws Exception {
		...
		mvc.perform(get("/api/books/")).andDo(print()).andExpect(status().isOk())
		        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
		        .andExpect(jsonPath("$[1].subtitle").value("Exile")).andExpect(jsonPath("$", hasSize(3)));
	}
...
//				.andExpect(jsonPath("$.subtitle == 'The Crystal Shard").doesNotExist());
	...
	public void GivenNewBook_WhenPostBook_ThenReturnCREATED() throws Exception {
		...
		mvc.perform(post("/api/books").contentType(MediaType.APPLICATION_JSON).characterEncoding("UTF-8")
		        .content(asJsonString(icewind1))).andDo(print()).andExpect(status().isCreated())
	...
	public void GivenModifiedBook_WhenPutBook_ThenReturnOK() throws Exception {
		...
		mvc.perform(put("/api/books/0").contentType(MediaType.APPLICATION_JSON).characterEncoding("UTF-8")
		        .content(asJsonString(icewind2Ammend))).andDo(print()).andExpect(status().isOk())
...
//		        .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("Invalid ID"));
